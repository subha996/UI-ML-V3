        ...
        try:
            if sample:
                if name=='boston': # boston dataset
                    ...
                    return  df.sample(frac=0.1) # returning 10% of total data for faster preprocessing
                    ...
                    return df.sample(frac=0.1)
            if custome:
                df = pd.read_csv(file_upload,nrows=100)
                return df
    ...
    try:
        if df is not None:
            ...
            if button:
                ...
                st.write("<h4 style='text-align: center;'>Training and Testing Prediction Plot</h4>", unsafe_allow_html=True)
